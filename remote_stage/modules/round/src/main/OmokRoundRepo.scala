...
import lila.omok.{ Color as OmokColor, Game as OmokGame, Move as OmokMove, OmokMoveDto, OmokOpeningDto, Pos, RuleSet }
...
final case class OmokOpeningState(
  ...
  def instruction(position: lila.omok.PositionSnapshot): String =
    ...
    else if candidateMode then "White removes candidate fifth moves until one remains."
    else
      position.ply match
        case 1 => "White may swap, or place the second move within 3x3 from H8."
        case 2 => "Black may swap, or place the third move within 5x5 from H8."
        case 3 => "White may swap, or place the fourth move within 7x7 from H8."
        case 4 if forceSimpleFifth => "Black places the fifth move within 9x9 from H8."
        case 4 => "Black may swap, place one fifth move within 9x9, or propose 10 fifth-move candidates."
        case 5 if !finalSwapUsed => "White may make the final swap, or continue Renju play."
