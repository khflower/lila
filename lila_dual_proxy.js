...
const HTTP_TARGET_PORT = 19663;
const WS_TARGET_PORT = 19664;
const LISTEN_PORT = 9663;
const omokSeatCookies = new Map();
const ASSET_VERSION = 'taraguchi-opening-20260407-1';
...
async function handleOmokClaim(req, res) {
  ...
  res.writeHead(303, {
    location: `http://${requestHost}/${fullId}`,
...
function proxyHttp(req, res) {
  ...
  const upstream = http.request(options, upstreamRes => {
    ...
    upstreamRes.on('end', () => {
      const body = Buffer.concat(chunks).toString('utf8');
      const normalizedHost = requestHost.replace(/^\[::1\]/, 'localhost');
      const rewritten = body
        ...
        .replaceAll(
          '/assets/compiled/round.STSFNM5F.js"',
          `/assets/compiled/round.STSFNM5F.js?v=${ASSET_VERSION}"`,
        )
        .replaceAll(
          `${normalizedHost}/assets/compiled/round.STSFNM5F.js"`,
          `${normalizedHost}/assets/compiled/round.STSFNM5F.js?v=${ASSET_VERSION}"`,
        )
        .replaceAll(
          '/assets/compiled/manifest.fb736118.js"',
          `/assets/compiled/manifest.fb736118.js?v=${ASSET_VERSION}"`,
        )
        .replaceAll(
          `${normalizedHost}/assets/compiled/manifest.fb736118.js"`,
          `${normalizedHost}/assets/compiled/manifest.fb736118.js?v=${ASSET_VERSION}"`,
        );
...
server.on('upgrade', (req, socket, head) => {
  ...
  const upstream = net.connect(WS_TARGET_PORT, '127.0.0.1', () => {
    const headers = {
      ...
      origin: `http://${req.headers.host || `localhost:${LISTEN_PORT}`}`,
      ...(seatCookie ? { cookie: mergeCookieHeader(req.headers.cookie, seatCookie) } : {}),
    };
...
const server4 = http.createServer(proxyHttp);
const server6 = http.createServer(proxyHttp);

attachUpgradeHandler(server4);
attachUpgradeHandler(server6);

server4.listen(LISTEN_PORT, '127.0.0.1', () => {
  console.log(`lila dual proxy listening on http://127.0.0.1:${LISTEN_PORT}`);
});

server6.listen(LISTEN_PORT, '::1', () => {
  console.log(`lila dual proxy listening on http://[::1]:${LISTEN_PORT}`);
});
