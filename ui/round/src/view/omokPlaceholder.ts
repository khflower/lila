...
import type { OmokOpening, OmokRoundPosition, SocketPlace } from '../interfaces';
...
const getTaraguchiOpeningFallback = (position?: OmokRoundPosition, ruleSet?: string): OmokOpening | undefined => {
  if (!position || (position.ruleSet ?? ruleSet) !== 'taraguchi10') return;
...
const getCurrentOpening = (ctrl: RoundController): OmokOpening | undefined => {
  const omok = ctrl.data.omok;
  const position = omok?.position;
  return position?.opening ?? getTaraguchiOpeningFallback(position, omok?.ruleset);
};
...
const sendPlaceNow = (ctrl: RoundController, pos: Key): void => {
  ctrl.socket.send('place', makePlacePayload(pos), { ackable: true });
};
...
const candidateBadgeOffset = (stoneRadius: number): number => Math.max(1.15, stoneRadius * 0.58);
const candidateBadgeRadius = (stoneRadius: number): number => Math.max(0.92, stoneRadius * 0.56);
...
const resolveBoardIndex = (relative: number, boardSize: number): number => {
  if (boardSize <= 1) return 0;
  const svgCoord = svgSize * relative;
  const normalized = (svgCoord - svgInset) / boardExtent;
  return clampIndex(normalized * (boardSize - 1), boardSize);
};
...
const bindBoardPlacement = (el: HTMLDivElement, ctrl: RoundController, boardSize: number): void => {
  ...
  const isCandidatePoint = (row: number, col: number): boolean =>
    !!getCurrentOpening(ctrl)?.candidates?.some(candidate => candidate.row === row && candidate.col === col);
...
export const renderOmokPlaceholder = (ctrl: RoundController): VNode | undefined => {
  ...
  for (const [candidateIndex, candidate] of opening?.candidates?.entries() || []) {
    ...
    boardSvgChildren.push(
      hl('circle.round__app__board__omok-placeholder__svg-candidate-badge', {
        attrs: {
          cx: cx - badgeOffset,
          cy: cy - badgeOffset,
          r: badgeRadius,
    ...
    boardSvgChildren.push(
      hl(
        'text.round__app__board__omok-placeholder__svg-candidate-label',
        {
          attrs: {
            x: cx - badgeOffset,
            y: cy - badgeOffset,
  ...
  const renderOpeningAction = (
    label: string,
    onClick: () => void,
    extraClass: Record<string, boolean> = {},
    attrs: Record<string, string> = { type: 'button' },
  ): VNode =>
    hl(
      'button.button.button-empty.round__app__board__omok-placeholder__opening-action',
      {
        class: extraClass,
        attrs,
        hook: bind('click', onClick, undefined, false),
      },
      label,
    );
        ...
        ? hl('div.round__app__board__omok-placeholder__opening', [
            ...
            hl('div.round__app__board__omok-placeholder__opening-actions', [
              ...
              opening.canSwap && canActInOpening
                ...
                : undefined,
              opening.canStartCandidates && canActInOpening
                ? renderOpeningAction('Propose 10', () => ctrl.socket.send('omok-candidates'))
                : undefined,
            ]),
          ])
        : undefined,
