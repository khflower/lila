...
const omokRuleInfo = ({ setupCtrl }: LobbyController): VNode =>
  hl('div.mselect.omok-rule-info.label-select', [
    hl('label.mselect__label', { attrs: { for: 'sf_omok_rule_set' } }, [
      hl('span.icon', '●'),
      hl('div.text', [
        hl('span.name', 'Omok'),
        hl('span.desc', 'Choose Renju for black forbidden moves, or Freestyle for unrestricted omok.'),
      ]),
    ]),
    hl(
      ...
      {
        on: {
          change: (e: Event) => setupCtrl.omokRuleSet((e.target as HTMLSelectElement).value as 'renju' | 'freestyle'),
        },
      },
      [
