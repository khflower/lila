...
final private class Rematcher(
  ...
  private def omokRuleSetOf(gameId: GameId): Fu[Option[RuleSet]] =
    omokRoundRepo.get(gameId).map(_.ruleSet) match
      case some @ Some(_) => fuccess(some)
      case None =>
        gameRepo.coll
          .one[Bdoc]($id(gameId), $doc(lila.game.Game.BSONFields.omok -> true))
          .map:
            _.flatMap:
              _.getAsOpt[Bdoc](lila.game.Game.BSONFields.omok)
                .flatMap(_.getAsOpt[String]("ruleSet"))
                .flatMap(parseOmokRuleSet)

  private def parseOmokRuleSet(raw: String): Option[RuleSet] =
