      ...
      .flatMap:
        case Some(pov) =>
          env.round.omokRoundRepo.get(pov.gameId).getOrElse {
            env.round.omokRoundRepo.put(pov.gameId, lila.round.OmokRoundState.initial(lila.omok.RuleSet.Renju))
          }
          // Render the claimed seat directly so black and white can stay in the
          // same browser session without fighting over anon ownership cookies.
          renderPlayer(pov)
        case None    => userC.tryRedirect(fullId.into(UserStr)).getOrElse(notFound)
