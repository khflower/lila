...
final private class RoundAsyncActor(
  ...
  val process: AsyncActor.ReceiveAsync =
          ...
          case Some(Right(applied)) =>
            applied.terminalStatus match
              case Some(terminalStatus) =>
                for
                  clockEvents <- stepOmokClockIfTurnChanged(pov.game, pov.color, applied.previous, applied.state)
                  _ <- fuccess:
              ...
              case None =>
                stepOmokClockIfTurnChanged(pov.game, pov.color, applied.previous, applied.state).map: clockEvents =>
                  version = version.map(_ + 1)
                  socketSend.exec(Protocol.Out.omokMove(version, applied.event))
                  clockEvents
    ...
    case p: HumanOmokSwap =>
      handle(p.playerId): pov =>
        val seat = assumedOmokTurnFor(pov.color)
