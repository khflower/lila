...
final class Env(
  ...
  private val scheduleExpiration = ScheduleExpiration: game =>
    Option.when(omokRoundRepo.get(game.id).isEmpty)(game)
      .flatMap(_.timeBeforeExpiration)
      .foreach: centis =>
  ...
  private lazy val storeOmokSidecar: OmokRoundRepo.StoreSidecar = stored =>
    gameRepo.coll.updateFieldUnchecked(
      $id(stored._id),
      lila.game.Game.BSONFields.omok,
      OmokStoredState.toBdoc(stored)
    )

  private lazy val clearOmokSidecar: OmokRoundRepo.ClearSidecar = gameId =>
    gameRepo.coll.updateUnchecked($id(gameId), $unset(lila.game.Game.BSONFields.omok))

  lazy val omokRoundRepo = OmokRoundRepo(storeOmokSidecar, clearOmokSidecar)
