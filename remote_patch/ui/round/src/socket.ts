  ...
  const handlers: SocketHandlers = {
    ...
    redirect(o?: RedirectTo) {
      if (o && typeof o === 'object' && 'url' in o) {
        if (o.cookie) {
          document.cookie = [
            encodeURIComponent(o.cookie.name) + '=' + o.cookie.value,
            '; max-age=' + o.cookie.maxAge,
            '; path=/',
            '; domain=' + location.hostname,
          ].join('');
        }
        site.unload.expected = true;
        location.href = '//' + location.host + '/' + o.url.replace(/^\//, '');
      } else ctrl.setRedirecting();
    },
