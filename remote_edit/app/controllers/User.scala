...
final class User(
  ...
  private def omokUserPage(username: UserStr, section: String = "overview")(using Context): Fu[Result] =
    Ok.page(views.omokPages.userProfile(username.value, section))

  private def omokPlayersPage(section: String = "overview")(using Context): Fu[Result] =
    Ok.page(views.omokPages.players(section))

  def tv(username: UserStr) = Open:
    omokUserPage(username, "tv")
  ...
  def search(term: String) = Open: _ ?=>
    UserStr.read(term) match
      ...
      case _              => Redirect(routes.User.list).toFuccess
  ...
  def download(username: UserStr) = OpenBody:
    omokUserPage(username, "download")
  ...
  def games(username: UserStr, filter: String, page: Int) = OpenBody:
    Reasonable(page):
      negotiate(
        omokUserPage(username, "games"),
        EnabledUser(username): u =>
          apiGames(u, filter, page)
      )
  ...
  def opponents = Auth { ctx ?=> me ?=>
    omokPlayersPage("opponents")
  }

  def perfStat(username: UserStr, perfKey: PerfKey) = Open:
    negotiate(
      omokUserPage(username, "perf"),
