...
final class Tv(env: Env, apiC: => Api, gameC: => Game) extends LilaController(env):
  ...
  private def omokGamesPage(channel: Option[String] = none)(using Context): Fu[Result] =
    Ok.page(views.omokPages.games(channel)).map(_.noCache)

  private def serveIndex(using Context) = omokGamesPage(Channel.Best.key.some)

  def onChannel(chanKey: String) = Open(serveChannel(chanKey))

  private def serveChannel(chanKey: String)(using Context) =
    omokGamesPage(chanKey.some)
  ...
  def frameDefault = Anon:
    Redirect(routes.Tv.games)

  def frame(chanKey: String) = Anon:
    Redirect(routes.Tv.gamesChannel(chanKey))
