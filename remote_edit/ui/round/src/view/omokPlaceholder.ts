  ...
  const renderCandidateModeButton = (mode: 'select' | 'remove', label: string): VNode =>
    hl(
      'button.button.button-empty.round__app__board__omok-placeholder__opening-action',
      {
        ...
        hook: bind('click', () => setCandidateMode(ctrl, mode), undefined, false),
      },
      label,
    );
                ...
                opening.canSwap && canActInOpening
                  ? hl(
                     'button.button.button-empty.round__app__board__omok-placeholder__opening-action',
                      {
                        attrs: { type: 'button' },
                        hook: bind('click', () => ctrl.socket.send('omok-swap'), undefined, false),
                    },
                    'Swap',
                  )
                : undefined,
                opening.canStartCandidates && canActInOpening
                  ? hl(
                     'button.button.button-empty.round__app__board__omok-placeholder__opening-action',
                      {
                        attrs: { type: 'button' },
                        hook: bind('click', () => ctrl.socket.send('omok-candidates'), undefined, false),
                    },
                    'Propose 10',
                  )
                : undefined,
