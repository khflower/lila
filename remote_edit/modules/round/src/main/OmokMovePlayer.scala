      ...
      if !previous.canStartCandidates then Left(s"[omok] $gameId cannot start candidate mode now")
        ...
        val state = previous.copy(
          opening = previous.opening
            .copy(candidateMode = true, candidateSelection = false)
...
object OmokMovePlayer:
    ...
    if !previous.opening.candidateMoves.exists(_.pos == move.pos) then Left(PlaceError(request, previous, OmokMoveError.OutOfBounds))
      ...
      if remaining.size == 1 then
        ...
        val finalOpening = appendMoveLog(
          previous,
          previous.opening
            .copy(candidateMode = false, candidateSelection = false, candidateMoves = Vector.empty)
            .log(OmokOpeningState.candidateRemoveEntry(previous.activeSeat, move, remaining.size)),
          finalMove,
        ...
        val state = previous.copy(
          opening = previous.opening
            .copy(candidateSelection = true, candidateMoves = remaining)
            .log(OmokOpeningState.candidateRemoveEntry(previous.activeSeat, move, remaining.size))
        )
  ...
  private def appendMoveLog(
    ...
    if previous.ruleSet == RuleSet.Taraguchi10 && previous.position.ply < 6 then
      opening.log(OmokOpeningState.moveEntry(previous.position.ply + 1, previous.activeSeat, move, suffix))
    else opening
