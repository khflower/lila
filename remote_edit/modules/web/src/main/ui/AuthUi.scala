...
final class AuthUi(helpers: Helpers):
  ...
  private def omokPage(title: String) = Page(title).copy(fullTitle = s"$title - $brandName".some)
  ...
  def login(form: Form[?], isRememberMe: Boolean = true)(using Option[ValidReferrer])(using Context) =
    ...
    omokPage(trans.site.signIn.txt())
  ...
  def signup(form: lila.core.security.HcaptchaForm[?], simple: Boolean)(using
    ...
    omokPage(trans.site.signUp.txt())
    ...
    omokPage(trans.site.signUp.txt())
      .css("bits.email-confirm"):
        main(cls := "page-small box box-pad signup-confirm")(
          h1(iconFlair(Flair("activity.party-popper")), "Welcome to Omok.dev"),
  ...
  def passwordReset(form: HcaptchaForm[?], fail: Option[String])(using Context) =
    omokPage(trans.site.passwordReset.txt())
      .css("bits.auth")
      .js(hcaptchaScript(form))
      .csp(_.withHcaptcha):
  ...
  def passwordResetSent(email: String)(using Context) =
    omokPage(trans.site.passwordReset.txt()).css("bits.auth"):
  ...
  def passwordResetConfirm(token: String, form: Form[?], ok: Option[Boolean] = None)(using
    ...
    omokPage(s"${me.username} - ${trans.site.changePassword.txt()}")
      .css("bits.auth")
  ...
  def magicLinkSent(using Context) =
    omokPage(trans.site.logInByEmail.txt()):
      main(cls := "page-small box box-pad")(
        boxTop(h1(cls := "is-green text", dataIcon := Icon.Checkmark)(trans.site.checkYourEmail())),
  ...
  def tokenLoginConfirmation(user: User, token: String)(using Context, Option[ValidReferrer]) =
    omokPage(s"Log in as ${user.username}").css("bits.form3"):
      main(cls := "page-small box box-pad")(
        ...
        postForm(action := addReferrer(routes.Auth.loginWithTokenPost(token).url))(
          form3.actions(
            ...
            submitButton(cls := "button")(s"${user.username} is my Omok.dev username, log me in")
          )
        )
      )
  ...
  def pubOrTor =
    omokPage("Public proxy"):
      main(cls := "page-small box box-pad")(
        boxTop(h1(cls := "text")("Ooops")),
        p("Sorry, you can't sign up to Omok.dev through Tor or public proxies!"),
        p("You can play, study, and use most Omok.dev features as an anonymous user.")
      )

  def logout(using Context) =
    omokPage(trans.site.logOut.txt()):
      main(cls := "page-small box box-pad")(
  ...
  private def agreement(form: play.api.data.Field, error: Boolean)(using Context) =
    div(cls := "agreement")(
      error.option(p:
        strong(cls := "error"):
          "You must agree to the Omok.dev policies listed below:"),
