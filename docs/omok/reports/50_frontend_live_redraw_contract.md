...
- `ui/round/src/ctrl.ts` mutates `data.omok.position` from `o.position`, exposes omok-first turn helpers, and still mirrors generic round turn metadata from that same position snapshot for wider round-shell compatibility.
- `ui/round/src/view/omokPlaceholder.ts` and `ui/round/src/view/omokState.ts` both redraw from `ctrl.data.omok.position`, with placement clickability and board-adjacent turn copy now resolving through the controller's omok-first turn helper.
...
- placement clickability through `ctrl.canPlaceOmok()`, which reads `data.omok.position.turn` before any generic round fallback;
- nearby turn presentation such as the board-adjacent turn clock copy and document title through `ctrl.currentTurnColor()` / `ctrl.isPlayerTurn()`, which also prefer `data.omok.position.turn`.
...
- Some broader round-shell behaviors still read mirrored generic state such as `data.game.player`, so `apiOmokMove(...)` continues updating that field for MVP compatibility.
